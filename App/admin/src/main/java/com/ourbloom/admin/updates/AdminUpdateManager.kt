...
object AdminUpdateManager {
    ...
    private const val PREFS_NAME = "ourbloom_admin_update_prefs"
    private const val KEY_DISMISSED_VERSION = "dismissed_version_code"

    // Direct raw APK fallback URL & server API
    ...
    private const val SERVER_VERSION_API = "https://our-bloom.onrender.com/api/admin/app/version"
    ...
    /**
     * Checks Firestore (and server fallback) for newer app version releases.
        ...
        activity.lifecycleScope.launch {
            try {
                ...
                try {
                    ...
                    Log.w(TAG, "Firestore version check note: ${e.message}")
                }

                // 2. If Firestore had no update or failed, check server endpoint
                if (updateInfo == null) {
                    try {
                        val serverUpdate = withContext(Dispatchers.IO) {
                            fetchServerVersionInfo()
                        }
                        if (serverUpdate != null && serverUpdate.latestVersionCode > BuildConfig.VERSION_CODE) {
                            updateInfo = serverUpdate
                        }
                    } catch (e: Exception) {
                        Log.d(TAG, "Server version check note: ${e.message}")
                    }
                }
                ...
                if (updateInfo != null) {
                    ...
                    val dismissedCode = prefs.getLong(KEY_DISMISSED_VERSION, 0L)

                    // If not a manual check and not a forced update, respect user's "Later" choice
                    if (!manualCheck && !updateInfo.forceUpdate && updateInfo.latestVersionCode <= dismissedCode) {
                        Log.d(TAG, "Update v${updateInfo.latestVersionName} previously dismissed, skipping prompt")
                        return@launch
                    }
                    ...
                    Toast.makeText(
                        activity,
                        "✨ OurBloom Admin is up to date (v${BuildConfig.VERSION_NAME} • Build ${BuildConfig.VERSION_CODE})",
                        Toast.LENGTH_LONG
                    ).show()
                }
    ...
    private fun fetchServerVersionInfo(): UpdateInfo? {
        val request = Request.Builder()
            .url(SERVER_VERSION_API)
        ...
        httpClient.newCall(request).execute().use { response ->
            ...
            val latestCode = json.optLong("latestVersionCode", 1L)
            val latestName = json.optString("latestVersionName", "1.0.0")
            val downloadUrl = json.optString("downloadUrl", DIRECT_RAW_APK_URL)
    ...
    private fun showUpdateDialog(activity: AppCompatActivity, updateInfo: UpdateInfo) {
        ...
        if (updateInfo.forceUpdate) {
            ...
            btnLater?.setOnClickListener {
                val prefs = activity.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                prefs.edit().putLong(KEY_DISMISSED_VERSION, updateInfo.latestVersionCode).apply()
                dialog.dismiss()
            }
        }
